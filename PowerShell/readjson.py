...
with open("/var/jenkins_home/jobs/SID_EtudeParcAppli/workspace/jenkins/EtudeParcAppli/hosts_paths.csv", "w") as f:
    writer = csv.writer(f)
    ...
    for env_data in app_data:
        try:
            for env_name, env_info in env_data.items():
                ...
                with open("/var/jenkins_home/jobs/SID_EtudeParcAppli/workspace/jenkins/EtudeParcAppli/hosts_paths.csv", "a", newline='') as f:
                    writer = csv.writer(f)
